...
| `icon`         | `icon`          | <span style="color:red">**[DEPRECATED]**</span> Use a native `<button>` instead.<br/><br/>Name of the icon to use. The `filled` version will be automatically used (if found) when the tab is `selected`.  | `string`                                                                     | `undefined`                            |
