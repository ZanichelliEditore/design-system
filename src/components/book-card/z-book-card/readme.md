...
| `hasMultipleCovers` | `has-multiple-covers` | Whether to show a stack of covers to represent multiple books. Only takes effect when `variant` is `portrait`: in `landscape` it is silently ignored and no cover stack is rendered, regardless of its value. | `boolean`                                               | `false`                    |
