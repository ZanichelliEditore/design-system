...
The width of the inner content of the topbar can be controlled to have a maximum value through the `contentmaxwidth` property.
...
<Preview>
  <Story name="ZAppTopbar with user dropdown, user not logged">
    {html`
      <z-app-topbar
        theme=${setTheme()}
        contentmaxwidth=${number('Content max width', null)}
