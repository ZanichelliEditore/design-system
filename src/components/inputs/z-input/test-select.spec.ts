...
describe("Suite test ZInput - select", () => {
  ...
  it("Test render ZInput aperta con elementi con autocomplete", async () => {
    const page = await newSpecPage({
      components: [ZInput],
      html: `<z-input htmlid="checkid" placeholder="select here" type="select" label="default" autocomplete items='[{"id":"item_0","name":"SELECT HERE questa opzione con etichetta lunga lunghissima","selected":false},{"id":"item_1","name":"primo elemento","selected":false}]'> </z-input>`
    });
    page.rootInstance.isOpen = true;
    await page.waitForChanges();
    expect(page.root).toEqualHtml(`
      <z-input type="select" htmlid="checkid" placeholder="select here" type="select" label="default" autocomplete items='[{"id":"item_0","name":"SELECT HERE questa opzione con etichetta lunga lunghissima","selected":false},{"id":"item_1","name":"primo elemento","selected":false}]'>
        <mock:shadow-root>
