...
export class ZSfPopover implements ComponentInterface {
  /**
  ...
  position?: SfPopoverPosition = SfPopoverPosition.TOP;
   ...
   */
  @State()
  currentPosition?: SfPopoverPosition;

  @Element() host: HTMLZSfPopoverElement;

  // Clockwise order of positions.
  private static readonly positionOrder: SfPopoverPosition[] = [
    SfPopoverPosition.TOP,
    SfPopoverPosition.TOP_RIGHT,
    SfPopoverPosition.TOP_LEFT,
    SfPopoverPosition.RIGHT,
    SfPopoverPosition.RIGHT_BOTTOM,
    SfPopoverPosition.RIGHT_TOP,
    SfPopoverPosition.BOTTOM,
    SfPopoverPosition.BOTTOM_LEFT,
    SfPopoverPosition.BOTTOM_RIGHT,
    SfPopoverPosition.LEFT,
    SfPopoverPosition.LEFT_TOP,
    SfPopoverPosition.LEFT_BOTTOM,
  ] as const;
  ...
  @Event()
  positionChange: EventEmitter<{position: SfPopoverPosition}>;
  ...
  closePopoverWithKeyboard(e: KeyboardEvent): void {
    if (this.closable && e.key === SfKeyboardCode.ESC) {
      e.stopPropagation();
      this.open = false;
    }
  }
  ...
  @Watch("position")
  validatePosition(newValue: SfPopoverPosition): void {
    if (
      !Object.values(SfPopoverPosition).includes(newValue as SfPopoverPosition) ||
      newValue === SfPopoverPosition.AUTO
    ) {
      newValue = SfPopoverPosition.TOP;
   ...
   */
  private getOptimalSfPopoverPosition(desiredPosition: SfPopoverPosition, availableSpace: Offsets): SfPopoverPosition {
    ...
    const fits = (pos: SfPopoverPosition): boolean => {
      switch (pos) {
        case SfPopoverPosition.TOP:
          return (
        ...
        case SfPopoverPosition.RIGHT_BOTTOM:
          return (
            availableSpace.right >= hostWidth &&
            this.hasEnoughSideSpace(availableSpace.bottom, hostHeight, boundElementHeight, offsetModifier)
          );
        case SfPopoverPosition.RIGHT_TOP:
          return (
            availableSpace.right >= hostWidth &&
            this.hasEnoughSideSpace(availableSpace.top, hostHeight, boundElementHeight, offsetModifier)
          );
        case SfPopoverPosition.BOTTOM:
          return (
            availableSpace.bottom >= hostHeight &&
            this.hasCenteredHorizontalSpace(availableSpace.left, availableSpace.right, hostWidth, boundElementWidth)
          );
        case SfPopoverPosition.BOTTOM_LEFT:
          return (
            availableSpace.bottom >= hostHeight &&
            this.hasEnoughSideSpace(availableSpace.left, hostWidth, boundElementWidth, offsetModifier)
          );
        case SfPopoverPosition.BOTTOM_RIGHT:
          return (
            availableSpace.bottom >= hostHeight &&
            this.hasEnoughSideSpace(availableSpace.right, hostWidth, boundElementWidth, offsetModifier)
          );
        case SfPopoverPosition.LEFT:
          return (
            availableSpace.left >= hostWidth &&
            this.hasCenteredVerticalSpace(availableSpace.top, availableSpace.bottom, hostHeight, boundElementHeight)
          );
        case SfPopoverPosition.LEFT_TOP:
          return (
            availableSpace.left >= hostWidth &&
            this.hasEnoughSideSpace(availableSpace.top, hostHeight, boundElementHeight, offsetModifier)
          );
        case SfPopoverPosition.LEFT_BOTTOM:
          return (
            availableSpace.left >= hostWidth &&
            this.hasEnoughSideSpace(availableSpace.bottom, hostHeight, boundElementHeight, offsetModifier)
          );
    ...
    const positions = ZSfPopover.positionOrder;
  ...
  private findBestFallbackPosition(availableSpace: Offsets): SfPopoverPosition {
    // Determine which horizontal and vertical direction has the most available space
    const bestHorizontalDirection =
      availableSpace.right >= availableSpace.left ? SfPopoverPosition.RIGHT : SfPopoverPosition.LEFT;

    const bestVerticalDirection =
      availableSpace.bottom >= availableSpace.top ? SfPopoverPosition.BOTTOM : SfPopoverPosition.TOP;
    ...
    return `${mainDirection}_${secondaryDirection}` as SfPopoverPosition;
  }
  ...
  private applyPositionStyles(position: SfPopoverPosition, availableSpace: Offsets): void {
    ...
    switch (position) {
      ...
      case SfPopoverPosition.TOP_RIGHT:
        ...
        hostStyle.left = `${leftBase - (position === SfPopoverPosition.TOP_RIGHT ? arrowModifier : 0)}px`;
        maxHeight = availableSpace.top - distanceFromBound;
        if (position === SfPopoverPosition.TOP_RIGHT) {
      ...
      case SfPopoverPosition.BOTTOM_RIGHT:
        ...
        hostStyle.left = `${leftBase - (position === SfPopoverPosition.BOTTOM_RIGHT ? arrowModifier : 0)}px`;
        maxHeight = availableSpace.bottom - distanceFromBound;
        if (position === SfPopoverPosition.BOTTOM_RIGHT) {
          maxWidth = availableSpace.right + boundElementWidth * offsetModifier;
        }
        break;
      ...
      case SfPopoverPosition.RIGHT:
      case SfPopoverPosition.RIGHT_BOTTOM:
        hostStyle.top = `${sideTopBase - (position === SfPopoverPosition.RIGHT_BOTTOM ? arrowModifier : 0)}px`;
        hostStyle.left = `${positionOffsets.left + boundElementWidth}px`;
        maxWidth = availableSpace.right - distanceFromBound;
        if (position === SfPopoverPosition.RIGHT) {
      ...
      case SfPopoverPosition.LEFT:
      case SfPopoverPosition.LEFT_BOTTOM:
        hostStyle.top = `${sideTopBase - (position === SfPopoverPosition.LEFT_BOTTOM ? arrowModifier : 0)}px`;
        hostStyle.right = `${positionOffsets.right + boundElementWidth}px`;
        maxWidth = availableSpace.left - distanceFromBound;
        if (position === SfPopoverPosition.LEFT_BOTTOM) {
          maxHeight = availableSpace.bottom + boundElementHeight * offsetModifier;
        }
        break;
    ...
    if (getSfDevice() !== SfDevice.MOBILE) {
  ...
  private setPosition(): void {
    ...
    const position = this.getOptimalSfPopoverPosition(this.position, availableSpace);
