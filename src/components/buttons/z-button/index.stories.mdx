...
In this special case we need to override `min-width` (achived with additional `square` prop). **Note:** `z-icon` drops its spacing accordingly.

| Prop name | Prop value              |
| --------- | ----------------------- |
| icon      | Any `z-icon` name       |
| square    | Attribute should be set |
  ...
  <Story name="With named slot">
    ...
    >
      <a slot="element" href="https://wikipedia.com" target="_blank">
        <z-icon name="${text("Icon name", "link")}"></z-icon>Go to Wikipedia
      </a>
    </z-button>`}
  </Story>
</Preview>
