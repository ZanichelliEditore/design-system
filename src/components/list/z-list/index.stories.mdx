import {html} from "lit-html";
import {Meta, Canvas, Story, Preview, Props, ArgsTable} from "@storybook/addon-docs";
import {DividerSize, ListSize, ListDividerType, ListType} from "../../../beans";
import "./index";

<Meta
  title="List/ZList"
  component="z-list"
  argTypes={{
    size: {
      control: {type: "select"},
      options: Object.values(ListSize),
      defaultValue: ListSize.MEDIUM,
    },
    listType: {
      control: {type: "select"},
      options: Object.values(ListType),
      defaultValue: ListType.NONE,
    },
  }}
/>

# ZList simple

<Canvas>
  <Story name="z-list-simple">
    {(args) =>
      html`
        <z-list list-type=${args.listType} size=${args.size}>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 1</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 2</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 3</z-list-element>
        </z-list>
      `
    }
  </Story>
</Canvas>

<ArgsTable of="z-list" />

# ZList with header division

<Canvas>
  <Story name="z-list-with-header-divider">
    {(args) =>
      html`<z-list>
        <z-list-group divider-type="header">
          <div slot="header-title">Gruppo 1</div>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 1</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 2</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 3</z-list-element>
        </z-list-group>
        <z-list-group divider-type="header">
          <div slot="header-title">Gruppo 2</div>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 1</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 2</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 3</z-list-element>
        </z-list-group>
      </z-list> `
    }
  </Story>
</Canvas>

# ZList with element divider

<Canvas>
  <Story name="z-list-with-element-divider">
    {(args) =>
      html`<z-list>
        <z-list-group divider-type="element">
          <div slot="header-title">Gruppo 1</div>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 1</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 2</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 3</z-list-element>
        </z-list-group>
        <z-list-group divider-type="element">
          <div slot="header-title">Gruppo 2</div>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 1</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 2</z-list-element>
          <z-list-element list-type=${args.listType} size=${args.size}>Elemento 3</z-list-element>
        </z-list-group>
      </z-list>`
    }
  </Story>
</Canvas>

# ZList with clickable elements

<Canvas>
  <Story name="z-list-with-clickable-elements">
    {(args) =>
      html`<z-list>
        <z-list-group>
          <z-list-element clickable>Elemento 1</z-list-element>
          <z-list-element clickable>Elemento 2</z-list-element>
          <z-list-element clickable>Elemento 3</z-list-element>
        </z-list-group>
      </z-list>`
    }
  </Story>
</Canvas>

# ZList with expandable elements

<Canvas>
  <Story name="z-list-with-expandable-elements">
    {(args) =>
      html`<z-list size="large">
        <z-list-group divider-type="element">
          <z-list-element expandable align-button="left" expandable-style="accordion"
            ><span>Elemento 1</span><span slot="inner-content">INNER CONTENT 1 BUTTON LEFT</span></z-list-element
          >
          <z-list-element expandable align-button="right" expandable-style="menu"
            ><span>Elemento 2</span><span slot="inner-content">INNER CONTENT 2</span></z-list-element
          >
          <z-list-element expandable align-button="right" expandable-style="menu"
            ><span>Elemento 3</span><span slot="inner-content">INNER CONTENT 3</span></z-list-element
          >
        </z-list-group>
      </z-list>`
    }
  </Story>
</Canvas>

# ZList with unordered index

<Canvas>
  <Story
    name="z-list-with-unordered-index"
    parameters={{
      controls: {
        exclude: ["size", "listType"],
      },
    }}
  >
    {(args) =>
      html`<z-list list-type="unordered">
        <z-list-element list-type="unordered">Elemento 1</z-list-element>
        <z-list-element list-type="unordered">Elemento 2</z-list-element>
        <z-list-element list-type="unordered">Elemento 3</z-list-element>
      </z-list>`
    }
  </Story>
</Canvas>

# ZList with ordered indez

<Canvas>
  <Story
    name="z-list-with-ordered-index"
    parameters={{
      controls: {
        exclude: ["size", "listType"],
      },
    }}
  >
    {(args) =>
      html`<z-list list-type="ordered">
        <z-list-element list-type="ordered">Elemento 1</z-list-element>
        <z-list-element list-type="ordered">Elemento 2</z-list-element>
        <z-list-element list-type="ordered">Elemento 3</z-list-element>
      </z-list>`
    }
  </Story>
</Canvas>

<ArgsTable of="z-list" />
