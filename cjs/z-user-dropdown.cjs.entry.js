...
const ZUserDropdown = class {
  ...
  renderDropdownMenu() {
    const colorClass = this.useInverseColors ? "inverse" : "";
    return (this.isMenuOpen && (index.h("ul", { class: colorClass }, this.isMobile && (index.h("li", { class: colorClass }, index.h("div", { class: `userfullname ${colorClass}` }, this.userFullName), index.h("div", { class: `useremail ${colorClass}` }, this.userEmail))), this.linkArray &&
      this.linkArray.map((link) => (index.h("li", { id: link.id, class: colorClass }, index.h("z-link", { textcolor: this.getZLinkTextcolor(), href: link.link, htmlid: link.id, target: link.target, icon: link.icon, onZLinkClick: this.emitDropdownMenuLinkClick }, link.label)))))));
  }
  render() {
