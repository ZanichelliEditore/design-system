...
class ZHeader {
    ...
    renderExtMenu(menuItems) {
        ...
        return (h("div", { id: "link-ext", class: "link-ext" }, menuItems.map((menuItem) => {
            ...
            return (h("span", { class: `link-ext-span ${this.ismyz && "myz"}` }, h("z-link", { id: id, htmlid: id, href: link, icon: icon, iswhite: this.ismyz ? true : false, target: "_blank", htmltabindex: 10 }, label)));
        })));
    }
...
class ZInput {
    ...
    renderCheckbox() {
        return (h("div", { class: "checkboxWrapper" }, h("input", { id: this.htmlid, type: "checkbox", name: this.name, checked: this.checked, value: this.value, disabled: this.disabled, readonly: this.readonly, onChange: () => this.handleCheckboxChange() }), h("label", { htmlFor: this.htmlid, class: `checkboxLabel ${this.labelafter ? "after" : "before"}` }, h("z-icon", { name: this.checked ? "checkbox-selected" : "checkbox-unchecked", "aria-hidden": true }), this.label && h("span", { innerHTML: this.label }))));
    }
    /* END checkbox */
    /* START select */
    renderSelect() {
