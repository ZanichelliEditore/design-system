...
class ZInput {
    ...
    renderSelectUl() {
        return (h("div", null, h("ul", { role: "listbox", tabindex: this.disabled || this.readonly ? -1 : 0, id: this.htmlid, "aria-activedescendant": this.value, class: `
          ...
          `, onClick: () => this.toggleSelectUl(), onKeyUp: (e) => handleKeyboardSubmit(e, this.toggleSelectUl), onKeyDown: (e) => this.arrowsSelectNav(e, this.selectedItem ? this.itemsList.indexOf(this.selectedItem) : -1) }, this.renderSelectedItem(), this.renderSelectItems())));
    }
    renderSelectedItem() {
    ...
    renderSelectItems() {
        if (!this.isOpen)
            return;
        return this.itemsList.map((item, key) => (h("li", { role: "option", tabindex: item.disabled ? -1 : 0, "aria-selected": !!item.selected, class: item.disabled && "disabled", id: `${this.htmlid}_${key}`, onClick: () => this.selectItem(item), onKeyUp: (e) => handleKeyboardSubmit(e, this.selectItem, item), onKeyDown: (e) => this.arrowsSelectNav(e, key) }, h("span", null, item.name))));
    }
    selectItem(item) {
        if (item.disabled)
            return;
...
const stylesCss$u = ":host{-webkit-box-sizing:border-box;box-sizing:border-box;position:relative;display:-ms-flexbox;display:flex;-ms-flex-direction:row;flex-direction:row;width:100%;max-width:100%;margin:0;padding:0;font-family:var(--dashboard-font);font-weight:var(--font-rg);list-style:none}@media only screen and (min-width: 768px){:host{padding:0 var(--basex2)}}";
