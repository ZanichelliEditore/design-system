...
class ZButtonSort {
    ...
    render() {
        return (h("button", { id: this.buttonid, class: this.isselected && "selected", onClick: () => this.emitButtonSortClick() }, h("label", null, this.sortasc ? this.label : this.desclabel, this.counter && ` (${this.counter})`), h("span", null, this.sortasc ? this.sortlabelasc : this.sortlabeldesc), h("z-icon", { name: "drop-up-down", width: 16, height: 16 })));
    }
}
    ...
    renderHeader() {
        return (h("div", { class: "header", onClick: () => this.closeComboBox(), onKeyDown: (ev) => {
                if (ev.keyCode === keybordKeyCodeEnum.SPACE)
                    ev.preventDefault();
            }, onKeyUp: (ev) => handleKeyboardSubmit(ev, this.closeComboBox), role: "button", tabindex: 0 }, h("h2", null, this.label, h("span", null, this.selectedCounter > 0 && ` (${this.selectedCounter})`)), h("z-icon", { name: "drop-down", width: 18, height: 18 })));
    }
    renderContent() {
        if (!this.isopen)
            return;
...
class ZFooter {
    ...
    renderFooterSection(id, group) {
        return (h("nav", null, h("div", { class: "header" }, h("h2", null, group.title), this.isMobile && (h("z-icon", { name: this.isOpen[id] ? "chevron-up" : "chevron-down", width: 16, height: 16, onClick: () => this.handleOnHeaderClick(id) }))), h("div", { class: "content" }, h("ul", { class: this.isOpen[id] ? "show" : "" }, group.items.map((item) => (h("li", null, h("a", { href: item.link, target: item.target ? item.target : "_blank" }, item.label))))))));
    }
    renderFooterTop() {
...
class ZHeader {
    ...
    renderMobileLoginDiv(userData) {
        return (h("div", { id: "mobile-login", class: "mobile-login" }, h("span", null, h("a", { class: "menu-item", id: "user-data", onClick: () => this.handleToggleMobileMenuItem("user-data"), role: "button" }, h("span", null, h("z-icon", { name: "user", height: 16, width: 16 }), userData.name), h("i", null)), this.renderUserData(userData))));
    }
...
class ZInfoBox {
    ...
    render() {
        return (h("div", { id: this.boxid }, h("slot", { name: "content" }), this.isclosable && h("z-icon", { name: "close", "data-action": "infoBoxClose", onClick: () => this.emitInfoBoxClose() })));
    }
}
...
class ZInput {
    ...
    renderCheckbox() {
        return (h("div", { class: "checkboxWrapper" }, h("input", { id: this.htmlid, type: "checkbox", name: this.name, checked: this.checked, value: this.value, disabled: this.disabled, readonly: this.readonly, required: this.required, onChange: () => this.handleCheckboxChange() }), h("label", { htmlFor: this.htmlid, class: `checkboxLabel ${this.labelafter ? "after" : "before"}` }, h("z-icon", { name: this.checked ? "checkbox-selected" : "checkbox-unchecked", "aria-hidden": true }), this.label && h("span", { innerHTML: this.label }))));
    }
    /* END checkbox */
    /* START radio */
    handleRadioChange() {
...
class ZMenuDropdown {
    ...
    render() {
        return (h("a", { class: this.retriveMenuClass(), role: "button", tabindex: "0", onFocus: () => {
            ...
            }, onKeyUp: (e) => handleKeyboardSubmit(e, this.handleToggle) }, h("div", { class: "container", onClick: () => this.handleToggle() }, h("z-icon", { name: "user", width: 14, height: 14 }), h("span", { class: "user" }, this.nomeutente), this.renderButtonMenu()), this.renderMenuOpen()));
    }
}
