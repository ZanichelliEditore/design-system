...
import { c as ButtonSize, I as InputType, K as KeyboardKeyCode, k as ListDividerType } from './index-968a240f.js';
import { h as handleKeyboardSubmit } from './utils-f2a0dac2.js';
...
const ZCombobox = class {
  ...
  renderItem(item, index, length) {
    return (h("z-myz-list-item", { id: item.id, listitemid: item.id, action: `combo-li-${this.inputid}`, underlined: index !== length - 1, class: this.size }, h("z-input", { type: InputType.CHECKBOX, checked: item.checked, htmlid: `combo-checkbox-${this.inputid}-${item.id}`, label: item.name, disabled: !item.checked && this.maxcheckableitems && this.maxcheckableitems === this.selectedCounter, size: this.size === ButtonSize.X_SMALL ? ButtonSize.SMALL : this.size })));
  }
  renderList(items) {
    if (!items) {
      return;
  ...
  renderCheckAll() {
    ...
    return (h("div", { class: "check-all-wrapper" }, h("z-input", { type: InputType.CHECKBOX, checked: allChecked, htmlid: `combo-checkbox-${this.inputid}-check-all`, label: allChecked ? this.uncheckalltext : this.checkalltext, disabled: this.maxcheckableitems && this.maxcheckableitems < this.itemsList.length, size: this.size === ButtonSize.X_SMALL ? ButtonSize.SMALL : this.size })));
  }
  render() {
