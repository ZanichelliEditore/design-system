...
const ZCardAlert = class {
    ...
    render() {
        return (h("div", { class: `relativeContainer ${this.retrieveClass()}` }, h("z-icon", { name: this.iconname, width: 18, height: 18, class: this.retrieveClass() }), h("span", { class: "contentText" }, this.contenttext), this.actiontext && !!this.actiontext.trim().length && (h("span", { role: "button", tabindex: "0", class: "contentAction", onClick: (e) => {
...
const stylesCss$6 = ":host{height:340px;width:auto}:host>div{height:340px;width:auto;display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center;-ms-flex-pack:center;justify-content:center;position:relative}";
...
const stylesCss$7 = "img{display:block;max-height:340px;margin:auto auto}img.faded{opacity:0.3}";
...
const ZFooter = class {
    ...
    renderFooterSection(id, group) {
        return (h("nav", null, h("div", { class: "header" }, h("h2", null, group.title), this.isMobile && (h("button", { onClick: () => this.handleOnHeaderClick(id) }, h("z-icon", { name: this.isOpen[id] ? "chevron-up" : "chevron-down", width: 16, height: 16 })))), h("div", { class: "content" }, h("ul", { class: this.isOpen[id] ? "show" : "" }, group.items.map((item) => (h("li", null, h("a", { href: item.link, target: item.target ? item.target : "_blank" }, item.label))))))));
    }
    renderFooterTop() {
    ...
    renderSocial() {
        ...
        return (h("ul", { class: "social" }, social.map((item) => (h("li", null, h("a", { href: item.link, target: "_blank" }, h("img", { src: item.icon, alt: item.description })))))));
    }
    renderCopyright() {
    ...
    renderBottomLinks() {
        const bottomLinks = this.jsonData.bottomLinks;
        if (!this.isMobile) {
            return (h("div", { class: "item bottom-links" }, h("ul", null, bottomLinks.map((item) => (h("li", null, h("a", { href: item.link, target: item.target ? item.target : "_blank" }, item.label)))))));
        }
    }
    renderFooterBottom() {
...
const ZHeader = class {
    ...
    renderExtMenu(menuItems) {
        ...
        return (h("div", { id: "link-ext", class: "link-ext" }, menuItems.map((menuItem) => {
            ...
            return (h("span", { class: `link-ext-span${this.ismyz ? " myz" : ""}` }, h("z-link", { id: id, htmlid: id, href: link, icon: icon, iswhite: !!this.ismyz, target: "_blank", htmltabindex: 10 }, label)));
        })));
    }
...
const stylesCss$h = ":host{line-height:0;vertical-align:middle}";
...
const stylesCss$i = ":host>div{display:-ms-flexbox;display:flex;-ms-flex-flow:row wrap;flex-flow:row wrap;-ms-flex-pack:start;justify-content:flex-start;font-family:Verdana, Arial, Helvetica, sans-serif;font-size:12px}:host>div>div{width:300px;margin:5px}:host>div>div>label{margin:0 10px}";
...
const ZInput = class {
    ...
    renderCheckbox() {
        return (h("div", { class: "checkboxWrapper" }, h("input", { id: this.htmlid, type: "checkbox", name: this.name, checked: this.checked, value: this.value, disabled: this.disabled, readonly: this.readonly, required: this.required, onChange: () => this.handleCheckboxChange() }), h("label", { htmlFor: this.htmlid, class: `checkboxLabel ${this.labelafter ? "after" : "before"}` }, h("z-icon", { name: this.checked ? "checkbox-checked" : "checkbox", "aria-hidden": true }), this.label && h("span", { innerHTML: this.label }))));
    }
    /* END checkbox */
    /* START radio */
    handleRadioChange() {
    ...
    renderRadio() {
        return (h("div", { class: "radioWrapper" }, h("input", { id: this.htmlid, type: "radio", name: this.name, checked: this.checked, value: this.value, disabled: this.disabled, readonly: this.readonly, onChange: () => this.handleRadioChange() }), h("label", { htmlFor: this.htmlid, class: `radioLabel ${this.labelafter ? "after" : "before"}` }, h("z-icon", { name: this.checked ? "radio-button-checked" : "radio-button", "aria-hidden": true }), this.label && h("span", { innerHTML: this.label }))));
    }
    /* END radio */
    /* START select */
    renderSelect() {
...
const stylesCss$l = ":host>label{display:block;color:var(--text-grey-800);font-family:var(--dashboard-font);font-weight:var(--font-sb);font-size:12px;line-height:16px;text-transform:uppercase;text-align:left;margin:var(--basex1) 0}:host>label.disabled{color:var(--text-grey-700)}";
...
const ZLink = class {
    ...
    render() {
        return (h("a", { id: this.htmlid, href: this.href ? this.href : null, class: `${this.isdisabled && "disabled"}
          ...
          ${this.iswhite && "white"}`, target: this.target, role: this.href ? "link" : "button", tabindex: this.htmltabindex, onClick: (e) => this.emitZLinkClick(e, this.htmlid) }, this.icon && h("z-icon", { name: this.icon, width: 12, height: 12 }), h("slot", null)));
    }
};
...
const stylesCss$q = ":host{font-family:var(--dashboard-font);font-weight:var(--font-rg)}:host>div{display:-ms-flexbox;display:flex;-ms-flex-direction:row;flex-direction:row;-ms-flex-pack:start;justify-content:flex-start}polygon{fill:var(--myz-red)}path{fill:var(--bg-white)}";
...
const ZMenuDropdown = class {
    ...
    render() {
        return (h("a", { class: this.retriveMenuClass(), role: "button", tabindex: "0", onFocus: () => {
            ...
            }, onKeyUp: (e) => handleKeyboardSubmit(e, this.handleToggle) }, h("div", { class: "container", onClick: () => this.handleToggle() }, h("z-icon", { name: "user-avatar", width: 14, height: 14 }), h("span", { class: "user" }, this.nomeutente), this.renderButtonMenu()), this.renderMenuOpen()));
    }
};
...
const stylesCss$w = ":host{-webkit-box-sizing:border-box;box-sizing:border-box;position:relative;display:-ms-flexbox;display:flex;-ms-flex-direction:row;flex-direction:row;width:100%;max-width:100%;margin:0;padding:0;font-family:var(--dashboard-font);font-weight:var(--font-rg);list-style:none}@media only screen and (min-width: 768px){:host{padding:0 var(--basex2)}}";
